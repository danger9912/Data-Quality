...
const ErrorModal = ({ show, onHide, fullscreen, data, filename }) => {
  ...
  const downloadExcel = async () => {
    ...
    // Add data rows
    data.forEach((item) => {
      const rowData = Object.values(item).filter(
        (value, index) => Object.keys(item)[index] !== "domainErrors"
      );
    ...
    worksheet.eachRow((row, rowNumber) => {
      row.eachCell((cell, colNumber) => {
        if (
          data[rowNumber - 2]?.domainErrors?.includes(
            Object.keys(data[rowNumber - 2])[colNumber - 1]
          )
        ) {
