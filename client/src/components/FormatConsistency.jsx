...
const FormatConsistency = () => {
  ...
  const sendFieldNames = async () => {
    try {
        if (selectedFilename) {
          ...
          const response = await axios.post(
            "http://localhost:3001/api/format/checkPK-format",
            {
              filename: selectedFilename,
  ...
  return(
    <>
    <h2>&nbsp;Format Consistency</h2>
    <center>
        <br /><h2>Primary Key Constraint</h2>
        <br /><input className="form-control uploadBtnInput" id="formFile" style={{ height: "2.5%", width: "355px" }} onChange={handleFileChange} type="file"/><br />
        <button type="button" className="btn btn-primary" onClick={fetchFieldNames}>
            Start Test
        </button>
        ...
        <button className="btn btn-primary mt-3" onClick={sendFieldNames} disabled={target.length === 0}>
            Check Validity
        </button>
      </center>
      ...
      <center>
        <br /><h2>Foreign Key Constraint</h2>
        <br /><input className="form-control uploadBtnInput" id="formFile" style={{ height: "2.5%", width: "355px" }} onChange={multiplehandleFileChange} type="file" multiple /><br />
        <button type="button" className="btn btn-primary" onClick={fetchForeignKeyValues}>
        Check Validity
        </button>
      </center>
      <div className="container">
