...
function ConfusionMatrix({ data }) {
  ...
  const createConfusionMatrix = (data) => {
    ...
    data.forEach(entry => {
      let actual = entry.actual.toLowerCase();
      let predicted = entry.pred.toLowerCase();
          ...
          {Object.keys(confusionMatrix).map(actualClass => (
            <tr key={actualClass}>
              ...
              {Object.keys(confusionMatrix).map(predictedClass => (
                <td key={predictedClass} style={{ border: '1px solid #dddddd', padding: '8px', backgroundColor: getBackgroundColor(confusionMatrix[actualClass][predictedClass] || 0) }}>
